...
function allOlder (f: Friend[]) : string[] {
    ...
    f.forEach(friend => {
        friend.age += 1
        const newString = `${friend.name} is now ${friend.age}`
...
function addColleague(cs: Colleague[], name : string, department: string, email : string) {
    const highestE = highestExtension(cs)
    ...
    const newColleague : Colleague =  {
        ...
        contact : {
            email : email,
            extension : highestE.contact.extension +1
        }
    }
  ...
  function addInterest(friend : Friend, interests : string) {
    if (!friend.interests) {
      friend.interests = []
    }
